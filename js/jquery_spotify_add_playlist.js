            ...
            return $.ajax(url, {
                ...
                success: function(r) {
                    ...
                    if (spotify_id.length === 0) {
                        $('#creating h1').text('Creando tu regalo');
                        return $.ajax(url, {
                            method: 'POST',
                            data: JSON.stringify({
                                'name': name,
                        ...
                        $('#creating h1').text('Buscando tu regalo');
            ...
            return getUsername(function(username) {
                return createPlaylist(username, g_name, function(playlist) {
                    return addTracksToPlaylist(username, playlist, g_tracks, function() {
                        $('#playlistlink').attr('href', 'https://play.spotify.com/user/' + username + '/playlist/' + playlist);
                        $('#playls').attr('src', 'https://embed.spotify.com/?uri=spotify%3Auser%3A' + username + '%3Aplaylist%3A' + playlist);
                        $('.js-fac').attr('href', 'https://www.facebook.com/sharer/sharer.php?u=https://play.spotify.com/user/' + username + '/playlist/' + playlist);
                        $('#creating').hide();
        ...
        return $.fn.spotify_add_to_playlist = function(options) {
            ...
            this.each(function() {
                ...
                if (location.hostname === "localhost") {
                    redirect_uri = window.location.href + 'callback/spotify-callback.html';
                } else {
                    redirect_uri = window.location.href + 'callback/callback.html';
                }
